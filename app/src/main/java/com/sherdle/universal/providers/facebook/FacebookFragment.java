...
public class FacebookFragment extends Fragment implements InfiniteRecyclerViewAdapter.LoadMoreListener {
	...
	private static String API_URL_END = "&date_format=U&fields=comments.limit(50).summary(1),likes.limit(0).summary(1),from,picture,message,story,id,created_time,full_picture,attachments{title,url_unshimmed,media{source},media_type}&limit=10";
		...
		try {
			...
			// parsing json object
             ...
             for (int i = 0; i < dataJsonArray.length(); i++) {
                 ...
                 JSONObject attachment = photoJson.getJSONObject("attachments").getJSONArray("data").getJSONObject(0);
                 post.type = attachment.getString("media_type");
                 ...
                 if (attachment.has("media") && attachment.getJSONObject("media").has("source"))
                	 post.link = attachment.getJSONObject("media").getString("source");
                 else 
                 ...
                 } else if (attachment.has("title")){
                	 post.caption =  attachment.getString("title");
                 } else {
                	 post.caption = "";
                 }
